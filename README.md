...
## Features (legend: * = done and - = not done)
	...
	* input
		...
		* voice recognition with reasonable blocking support for not hearing active speaking puppets (npc doesn't hear npc)
		- interruptions could be better supported more formally; an escape key and detection of voice interruptions
		- new conversations should pre-empt old conversations - right now conversations pile up as performances
		- a race condition where a later prompt was played earlier; the pipelines don't have any intelligence
	...
	* facial performances
		...
		* optional vrm support with a blender python script that can be run in a commandline headless mode for bulk conversions
		- vrm gaze ... rigs will probably need updated gaze targets for head and eyes in order to work correctly
...
## Other
	...
	* polish
		- remove 150 ms hack?
		- remove lipsync split on server?
		- interruption support - see https://x.com/trydaily/status/1815530613434417241 for better voice stuff timing and interruption ?
		- story concepts could be improved for scenarios; like a librarian or something in actual roles not lorem ipsum
		- make sure that text chat is networked
		- explicitly support multi or solo player modes - with reasoning being done on server if desired
		- push to a website as a standalone example, with transparent backdrop; document as a useful tool
...
* facial performances
	...
	- Synchronizing the mouth movements with the audio helps provide a sense of presence and aliveness that makes an avatar more engaging and evocative.
	...
	- We use a simple speech to text tool (whisper) to understand spoken text and generate visemes (facial morph targets) to play back
...
* speech recognition
	...
	- we use whisper to convert audio to text in order to extract phonemes and visemes to later play back on the puppet
...
* Other market participants
	...
	- inworld.io ... nice articulated puppets but re-licensing terms are restrictive (even on their source code)
	- other
