  ...
  resetLips() {
    ...
    this.visemeNames.forEach( x => {
      this.morphs.forEach( y => {
        let ndx = y.morphTargetDictionary['viseme_'+x];
        if(ndx == undefined) ndx = y.morphTargetDictionary['mouthFunnel']
        if ( ndx !== undefined ) {
          y.morphTargetInfluences[ndx] = 0;
        } else {
          console.log("cannot find",x)
          console.log(y.morphTargetDictionary)
        }
      });
    });
  }
