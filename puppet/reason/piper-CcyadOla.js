var ft = (() => {
  ...
  return typeof __filename < "u" && (Q = Q || __filename), function(Fe = {}) {
    ...
    o.ready = new Promise((e, a) => {
      ...
      if (!(o.ENVIRONMENT_IS_PTHREAD || o.$ww)) {
        var e = function(a) {
          ...
          typeof o.locateFilePackage == "function" && !o.locateFile && (o.locateFile = o.locateFilePackage, C("warning: you defined Module.locateFilePackage, that has been renamed to Module.locateFile (using your locateFilePackage for now)"));
          ...
          function y() {
            ...
            g.prototype = { requests: {}, open: function(b, S) {
            ...
            for (var p = a.files, _ = 0; _ < p.length; ++_)
              new g(p[_].start, p[_].end, p[_].audio || 0).open("GET", p[_].filename);
            function E(b) {
    ...
    } else (Ae || q) && (q ? N = self.location.href : typeof document < "u" && document.currentScript && (N = document.currentScript.src), Q && (N = Q), N.indexOf("blob:") !== 0 ? N = N.substr(0, N.replace(/[?#].*/, "").lastIndexOf("/") + 1) : N = "", G = (e) => {
    ...
    var de = (e) => {
      ...
      L(`Assertion failed: ${W(e)}, at: ` + [a ? W(a) : "unknown filename", t, r ? W(r) : "unknown function"]);
    };
    function ua(e) {
    ...
    var je = 0, va = (e, a, t) => {
    ...
    }, ka = (e) => (k[$e() >> 2] = e, e), P = { isAbs: (e) => e.charAt(0) === "/", splitPath: (e) => {
      var a = /^(\/?|)([\s\S]*?)((?:\.{1,2}|[^\/]+?|)(\.[^.\/]*|))(?:[\/]*)$/;
      ...
      return e = P.normalizeArray(e.split("/").filter((r) => !!r), !a).join("/"), !e && !a && (e = "."), e && t && (e += "/"), (a ? "/" : "") + e;
    }, dirname: (e) => {
      ...
      L("initRandomDevice");
    }, Ue = (e) => (Ue = ha())(e), O = { resolve: function() {
      ...
      return e = P.normalizeArray(e.split("/").filter((s) => !!s), !a).join("/"), (a ? "/" : "") + e || ".";
    }, relative: (e, a) => {
...
`));
    ...
    }, n = { root: null, mounts: [], devices: {}, streams: [], nextInode: 1, nameTable: null, currentPath: "/", initialized: !1, ignorePermissions: !0, ErrnoError: null, genericErrors: {}, filesystems: null, syncFSRequests: 0, lookupPath(e, a = {}) {
      ...
      return n.FSStream || (n.FSStream = function() {
        this.shared = {};
      }, n.FSStream.prototype = {}, Object.defineProperties(n.FSStream.prototype, { object: { get() {
        return this.node;
      }, set(t) {
        this.node = t;
      } }, isRead: { get() {
      ...
      typeof e == "function" && (a = e, e = !1), n.syncFSRequests++, n.syncFSRequests > 1 && C(`warning: ${n.syncFSRequests} FS.syncfs operations in flight at once, probably just doing extra work`);
      ...
      if (!r.node_ops.setattr)
        throw new n.ErrnoError(63);
      r.node_ops.setattr(r, { mode: a & 4095 | r.mode & -4096, timestamp: Date.now() });
    }, lchmod(e, a) {
      n.chmod(e, a, !0);
    }, fchmod(e, a) {
      ...
      var l = n.createStream({ node: r, path: n.getPath(r), flags: a, seekable: !0, position: 0, stream_ops: r.stream_ops, ungotten: [], error: !1 });
      ...
      if (!d)
        ...
        throw new n.ErrnoError(70);
      var l = e.stream_ops.write(e, a, t, r, s, i);
      ...
      n.mkdir("/dev"), n.registerDevice(n.makedev(1, 3), { read: () => 0, write: (r, s, i, d, l) => d }), n.mkdev("/dev/null", n.makedev(1, 3)), B.register(n.makedev(5, 0), B.default_tty_ops), B.register(n.makedev(6, 0), B.default_tty1_ops), n.mkdev("/dev/tty", n.makedev(5, 0)), n.mkdev("/dev/tty1", n.makedev(6, 0));
      ...
      n.mkdir("/proc/self/fd"), n.mount({ mount() {
        ...
        return a.node_ops = { lookup(t, r) {
          var s = +r, i = n.getStreamChecked(s), d = { parent: null, mount: { mountpoint: "fake" }, node_ops: { readlink: () => i.path } };
          return d.parent = d, d;
        } }, a;
      } }, {}, "/proc/self/fd");
      ...
      return n.registerDevice(d, { open(l) {
        l.seekable = !1;
      }, close(l) {
      ...
      if (i.prototype.get = function(p) {
        ...
        b || (f = _);
        var c = (D, x) => {
          if (D > x) throw new Error("invalid range (" + D + ", " + x + ") or no bytes requested!");
          if (x > _ - 1) throw new Error("only " + _ + " bytes available! programmer error!");
          var R = new XMLHttpRequest();
          if (R.open("GET", t, !1), _ !== f && R.setRequestHeader("Range", "bytes=" + D + "-" + x), R.responseType = "arraybuffer", R.overrideMimeType && R.overrideMimeType("text/plain; charset=x-user-defined"), R.send(null), !(R.status >= 200 && R.status < 300 || R.status === 304)) throw new Error("Couldn't load " + t + ". Status: " + R.status);
      ...
      l.contents ? m.contents = l.contents : l.url && (m.contents = null, m.url = l.url), Object.defineProperties(m, { usedBytes: { get: function() {
    ...
    } }, F = { DEFAULT_POLLMASK: 5, calculateAt(e, a, t) {
      if (P.isAbs(a))
        return a;
      var r;
      if (e === -100)
      ...
      k[t >> 2] = r.dev, k[t + 4 >> 2] = r.mode, A[t + 8 >> 2] = r.nlink, k[t + 12 >> 2] = r.uid, k[t + 16 >> 2] = r.gid, k[t + 20 >> 2] = r.rdev, M = [r.size >>> 0, (u = r.size, +Math.abs(u) >= 1 ? u > 0 ? +Math.floor(u / 4294967296) >>> 0 : ~~+Math.ceil((u - +(~~u >>> 0)) / 4294967296) >>> 0 : 0)], k[t + 24 >> 2] = M[0], k[t + 28 >> 2] = M[1], k[t + 32 >> 2] = 4096, k[t + 36 >> 2] = r.blocks;
      var s = r.atime.getTime(), i = r.mtime.getTime(), d = r.ctime.getTime();
      return M = [Math.floor(s / 1e3) >>> 0, (u = Math.floor(s / 1e3), +Math.abs(u) >= 1 ? u > 0 ? +Math.floor(u / 4294967296) >>> 0 : ~~+Math.ceil((u - +(~~u >>> 0)) / 4294967296) >>> 0 : 0)], k[t + 40 >> 2] = M[0], k[t + 44 >> 2] = M[1], A[t + 48 >> 2] = s % 1e3 * 1e3, M = [Math.floor(i / 1e3) >>> 0, (u = Math.floor(i / 1e3), +Math.abs(u) >= 1 ? u > 0 ? +Math.floor(u / 4294967296) >>> 0 : ~~+Math.ceil((u - +(~~u >>> 0)) / 4294967296) >>> 0 : 0)], k[t + 56 >> 2] = M[0], k[t + 60 >> 2] = M[1], A[t + 64 >> 2] = i % 1e3 * 1e3, M = [Math.floor(d / 1e3) >>> 0, (u = Math.floor(d / 1e3), +Math.abs(u) >= 1 ? u > 0 ? +Math.floor(u / 4294967296) >>> 0 : ~~+Math.ceil((u - +(~~u >>> 0)) / 4294967296) >>> 0 : 0)], k[t + 72 >> 2] = M[0], k[t + 76 >> 2] = M[1], A[t + 80 >> 2] = d % 1e3 * 1e3, M = [r.ino >>> 0, (u = r.ino, +Math.abs(u) >= 1 ? u > 0 ? +Math.floor(u / 4294967296) >>> 0 : ~~+Math.ceil((u - +(~~u >>> 0)) / 4294967296) >>> 0 : 0)], k[t + 88 >> 2] = M[0], k[t + 92 >> 2] = M[1], 0;
    }, doMsync(e, a, t, r, s) {
    ...
    function Da(e, a, t) {
      try {
        ...
        for (var s = 280, i = 0, d = n.llseek(r, 0, 1), l = Math.floor(d / s); l < r.getdents.length && i + s <= t; ) {
          ...
          M = [m >>> 0, (u = m, +Math.abs(u) >= 1 ? u > 0 ? +Math.floor(u / 4294967296) >>> 0 : ~~+Math.ceil((u - +(~~u >>> 0)) / 4294967296) >>> 0 : 0)], k[a + i >> 2] = M[0], k[a + i + 4 >> 2] = M[1], M = [(l + 1) * s >>> 0, (u = (l + 1) * s, +Math.abs(u) >= 1 ? u > 0 ? +Math.floor(u / 4294967296) >>> 0 : ~~+Math.ceil((u - +(~~u >>> 0)) / 4294967296) >>> 0 : 0)], k[a + i + 8 >> 2] = M[0], k[a + i + 12 >> 2] = M[1], V[a + i + 16 >> 1] = 280, T[a + i + 18 >> 0] = v, qe(y, a + i + 19, 256), i += s, l += 1;
        }
          ...
          case 21508: {
            ...
            if (r.tty.ops.ioctl_tcsets) {
              for (var i = F.getp(), l = k[i >> 2], m = k[i + 4 >> 2], v = k[i + 8 >> 2], y = k[i + 12 >> 2], h = [], d = 0; d < 32; d++)
                h.push(T[i + d + 17 >> 0]);
              return r.tty.ops.ioctl_tcsets(r.tty, a, { c_iflag: l, c_oflag: m, c_cflag: v, c_lflag: y, c_cc: h });
            }
            return 0;
          }
          case 21519: {
    ...
    var Na = !0, xa = () => Na, Ca = () => {
      ...
      if (!Z.strings) {
        var e = (typeof navigator == "object" && navigator.languages && navigator.languages[0] || "C").replace("-", "_") + ".UTF-8", a = { USER: "web_user", LOGNAME: "web_user", PATH: "/", PWD: "/", HOME: "/home/web_user", LANG: e, _: Ba() };
        for (var t in Ee)
    ...
    function Za(e, a, t, r, s) {
      ...
      try {
        ...
        return n.llseek(d, i, r), M = [d.position >>> 0, (u = d.position, +Math.abs(u) >= 1 ? u > 0 ? +Math.floor(u / 4294967296) >>> 0 : ~~+Math.ceil((u - +(~~u >>> 0)) / 4294967296) >>> 0 : 0)], k[s >> 2] = M[0], k[s + 4 >> 2] = M[1], d.getdents && i === 0 && r === 0 && (d.getdents = null), 0;
      } catch (l) {
        if (typeof n > "u" || l.name !== "ErrnoError") throw l;
        return l.errno;
      }
    ...
    var fe = (e) => e % 4 === 0 && (e % 100 !== 0 || e % 400 === 0), at = (e, a) => {
      ...
      var s = A[r + 40 >> 2], i = { tm_sec: k[r >> 2], tm_min: k[r + 4 >> 2], tm_hour: k[r + 8 >> 2], tm_mday: k[r + 12 >> 2], tm_mon: k[r + 16 >> 2], tm_year: k[r + 20 >> 2], tm_wday: k[r + 24 >> 2], tm_yday: k[r + 28 >> 2], tm_isdst: k[r + 32 >> 2], tm_gmtoff: k[r + 36 >> 2], tm_zone: s ? W(s) : "" }, d = W(t), l = { "%c": "%a %b %d %H:%M:%S %Y", "%D": "%m/%d/%y", "%F": "%Y-%m-%d", "%h": "%b", "%r": "%I:%M:%S %p", "%R": "%H:%M", "%T": "%H:%M:%S", "%x": "%m/%d/%y", "%X": "%H:%M:%S", "%Ec": "%c", "%EC": "%C", "%Ex": "%m/%d/%y", "%EX": "%H:%M:%S", "%Ey": "%y", "%EY": "%Y", "%Od": "%d", "%Oe": "%e", "%OH": "%H", "%OI": "%I", "%Om": "%m", "%OM": "%M", "%OS": "%S", "%Ou": "%u", "%OU": "%U", "%OV": "%V", "%Ow": "%w", "%OW": "%W", "%Oy": "%y" };
      for (var m in l)
        d = d.replace(new RegExp(m, "g"), l[m]);
      var v = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"], y = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
      function h(f, c, z) {
      ...
      var b = { "%a": (f) => v[f.tm_wday].substring(0, 3), "%A": (f) => v[f.tm_wday], "%b": (f) => y[f.tm_mon].substring(0, 3), "%B": (f) => y[f.tm_mon], "%C": (f) => {
        var c = f.tm_year + 1900;
        return g(c / 100 | 0, 2);
      }, "%d": (f) => g(f.tm_mday, 2), "%e": (f) => h(f.tm_mday, 2, " "), "%g": (f) => E(f).toString().substring(2), "%G": (f) => E(f), "%H": (f) => g(f.tm_hour, 2), "%I": (f) => {
        var c = f.tm_hour;
        return c == 0 ? c = 12 : c > 12 && (c -= 12), g(c, 2);
      }, "%j": (f) => g(f.tm_mday + at(fe(f.tm_year + 1900) ? Ie : Ye, f.tm_mon - 1), 3), "%m": (f) => g(f.tm_mon + 1, 2), "%M": (f) => g(f.tm_min, 2), "%n": () => `
`, "%p": (f) => f.tm_hour >= 0 && f.tm_hour < 12 ? "AM" : "PM", "%S": (f) => g(f.tm_sec, 2), "%t": () => "	", "%u": (f) => f.tm_wday || 7, "%U": (f) => {
    ...
    Object.defineProperties(We.prototype, { read: { get: function() {
    ...
    var dt = { a: ga, b: va, e: Pa, r: Da, v: Aa, f: Ma, p: Ra, o: za, q: Ta, j: xa, h: Ca, g: La, k: Oa, n: Ua, s: Ha, t: Ia, d: $a, c: Xa, u: Ka, l: Za, i: et, m: st }, j = ca(), Ge = o._main = (e, a) => (Ge = o._main = j.y)(e, a), $e = () => ($e = j.z)(), Se = (e) => (Se = j.B)(e), Xe = (e) => (Xe = j.C)(e);
